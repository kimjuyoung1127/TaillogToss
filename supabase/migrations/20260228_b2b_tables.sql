-- B2B-001: B2B 확장 테이블 10개 + 기존 테이블 ALTER 3개 + RLS 헬퍼 3개
-- SCHEMA-B2B.md v1.0.0 기준
...
-- NULL-safe uniqueness: active만 유니크
...
-- ============================================================
-- 2. 기존 테이블 수정 (3개 — users, behavior_logs, media_assets)
-- ============================================================

-- 2.1 users.role CHECK 확장 (4종)
ALTER TABLE public.users DROP CONSTRAINT IF EXISTS users_role_check;
ALTER TABLE public.users ADD CONSTRAINT users_role_check
  CHECK (role IN ('user', 'trainer', 'org_owner', 'org_staff'));

-- 2.2 behavior_logs: B2B 필드 추가
...
-- PII 감사 로그 테이블 (get_parent_contact에서 사용)
...
-- PII 읽기: SECURITY DEFINER RPC + 감사 로그
...
-- PII 삭제 CRON: discharged 90일 초과 자동 삭제
...
-- daily_reports: INSERT/UPDATE/SELECT
...
-- parent_interactions: INSERT/UPDATE
...
-- org_subscriptions: 조직 owner/본인만
