/**
 * login-with-toss main — Deno serve 엔트리포인트.
 * Parity: AUTH-001
 */
...
import { handleLoginWithToss, type LoginWithTossRequest } from './index.ts';
...
if (!edgeRuntime?.serve) {
  throw new Error('Deno runtime is required for login-with-toss main entrypoint');
}
...
edgeRuntime.serve(async (request: Request) => {
  ...
  const body = await parseRequestJson<LoginWithTossRequest>(request);
  if (!body) {
    return toJsonResponse(fail('VALIDATION_ERROR', 'Invalid JSON body', 400));
  }

  const context = buildEdgeContext(request);
  const result = await handleLoginWithToss(body, context);
