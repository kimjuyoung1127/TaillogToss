...
│          │     │          │     │              │     │               │
│토스 OAuth │     │3 benefit │     │이름→품종→나이 │     │AI 요약 + 티저 │
│브릿지     │     │carousel  │     │→체중→중성화   │     │Skeleton 블러  │
│          │     │"90초 시작"│     │→문제행동→목표 │     │광고/PRO CTA   │
└─────────┘     └──────────┘     └──────────────┘     └───────┬───────┘
                                                              │
                                                              ▼
                                                    ┌───────────────┐
...
**보상형 광고 퍼널**: Skeleton 블러 → TextButton("광고 보고 열기") → AdMob Rewarded → 1회 해제 → "매번 광고 귀찮다 → PRO 구독" 자연 전환
...
```
┌─────────────────────────┐
│                    [✕]  │  ← Navbar.CloseButton (건너뛰기)
├─────────────────────────┤
│                         │
│  ┌───────────────────┐  │
│  │  [Carousel]       │  │  ← Carousel(3 benefit cards)
│  │                   │  │
│  │  Card 1:          │  │
│  │  [Asset: 강아지]  │  │     Asset(type="lottie")
│  │  "반려견 행동,     │  │
│  │   90초면 기록 끝"  │  │
│  │                   │  │
│  │  ● ○ ○            │  │  ← 페이지 인디케이터
...
│                         │
│  TextButton("이미 계정이 │
│   있어요")              │  ← TextButton → login 화면
└─────────────────────────┘
```

**사용 컴포넌트**: `Navbar(.CloseButton)`, `Carousel`, `Asset`(Lottie), `BottomCTA`, `Button`, `TextButton`
**전환**: "시작하기" → survey(Step 1), "이미 계정이 있어요" → login
**카피 원칙**: 숫자 강조("90초"), 결과 중심("확실한 변화"), 부담 최소화
...
│  │ "광고 보고 전체    │  │     → AdMob Rewarded → 1회 잠금 해제
│  │  분석 보기"        │  │
│  └───────────────────┘  │
│                         │
├─────────────────────────┤
│  ┌───────────────────┐  │
│  │ PRO로 모든 분석    │  │  ← BottomCTA > Button(primary)
│  │ 잠금 해제          │  │     → subscription 화면
│  └───────────────────┘  │
│  TextButton("먼저 기록  │
│   시작하기")            │  ← TextButton → dashboard
└─────────────────────────┘
```

**사용 컴포넌트**: `Navbar(.BackButton, .Title)`, `Asset`, `Badge`, `ProgressBar`, `Border`, `ListHeader`, `Skeleton`(블러), `TextButton`, `BottomCTA`, `Button`
**보상형 광고(R1)**: "광고 보고 전체 분석 보기" → AdMob Rewarded 시청 → Skeleton 해제 → 1회 열기
**PRO CTA**: "PRO로 모든 분석 잠금 해제" → subscription 화면
**이탈 안전망**: "먼저 기록 시작하기" → dashboard (무료 진입 보장)
...
**사용 컴포넌트**: `Navbar(.CloseButton)`, `Asset`(Lottie), `Checkbox` × 3, `Border`, `BottomCTA`, `Button`, `TextButton`
...
│  │ ₩9,900 (1회=₩330) │  │
│  ├───────────────────┤  │
│  │ 훈련사 PRO         │  │  ← BoardRow + Badge("B2B")
│  │ ₩19,900/월         │  │
│  │ 담당견 관리+리포트  │  │
│  ├───────────────────┤  │
│  │ 센터 PRO           │  │  ← BoardRow + Badge("B2B")
│  │ ₩49,900/월         │  │
│  │ 직원 관리+벌크 기록 │  │
│  └───────────────────┘  │
│                         │
...
**사용 컴포넌트**: `Navbar(.BackButton, .Title)`, `Top`, `ListHeader`, `TableRow` × 5, `Border`, `SegmentedControl`, `BoardRow` × 4, `Badge`("인기", "B2B"), `ListFooter`, `BottomCTA`, `Button`
**IAP 로직**: "구독하기" → Toss IAP SDK → 결제 성공 → Edge Function `verify-iap-order` → 권한 갱신
**B2B 자동 전환**:
- 훈련사 플랜: `verify-iap-order` → `users.role='trainer'` → 대시보드 [운영] 탭 활성화
- 센터 플랜: `organizations` 생성 + `users.role='org_owner'` → 직원 초대 플로우
**구독 만료**: role 유지, 기록/리포트 생성 차단, 기존 데이터 열람 30일 유예
...
**사용 컴포넌트**: `Navbar(.BackButton, .Title)`, `ProgressBar`, `Asset`(image/video), `ListHeader` × 2, `Checkbox` × N, `Border`, `TextField`(multiline), `BottomCTA`, `Button`
...
│  │ 🚨 행동 문제                 │   │  ← ListHeader
│  │                             │   │
│  │ [짖음] [입질] [배변실수]     │   │  ← Chip × 6 (원탭 = 즉시 저장)
│  │ [분리불안] [흥분] [기타]     │   │     intensity=3 고정
│  └─────────────────────────────┘   │
│                                     │
│  ┌─────────────────────────────┐   │
│  │ 📋 일상 활동                 │   │  ← ListHeader
...
**빠른 탭 UX**:
- 행동 문제 6칩: 원탭 = 즉시 저장 (2초), intensity=3 고정
...
│  │ "광고 보고 오늘의  │  │     → AdMob Rewarded → 1회 잠금 해제
│  │  코칭 열기"        │  │
│  └───────────────────┘  │
│                         │
├─────────────────────────┤
...
**보상형 광고(R3)**: "광고 보고 오늘의 코칭 열기" → AdMob Rewarded → Skeleton 해제 → 1회 열기
...
| 7 | survey-result | notification | "기록 시작하기" 또는 PRO CTA 후 | → slide right | — |
| 8 | survey-result | subscription | "PRO" 버튼 | → slide right | — |
| 9 | notification | dashboard | "허용" 또는 "나중에" | → slide right | — |
| 10 | dashboard | quick-log | "+ 빠른 기록" CTA | ↑ bottom sheet | onboarding |
| 11 | quick-log | dashboard | 저장 완료 | ↓ close sheet | — |
| 12 | dashboard | analysis | [분석] 탭 | → tab switch | onboarding |
| 13 | dashboard | training-academy | [훈련] 탭 | → tab switch | onboarding |
| 14 | dashboard | ops-today | [운영] 탭 | → tab switch | role(trainer+) |
| 15 | dashboard | dog-switcher | 반려견 이름 탭 | ↑ bottom sheet | — |
| 16 | dashboard | settings | ⚙️ 아이콘 | → slide right | — |
| 17 | dashboard | coaching-result | "오늘의 코칭" 영역 탭 | → slide right | onboarding |
| 18 | analysis | coaching-result | "AI 코칭 보기" | → slide right | feature(3건+) |
| 19 | coaching-result | training-academy | "훈련 시작하기" | → slide right | — |
| 20 | coaching-result | subscription | "PDF 리포트(PRO)" | → slide right | — |
| 21 | training-academy | training-detail | 미션 카드 탭 | → slide right | — |
| 22 | training-detail | training-academy | "미션 완료" 또는 BackButton | ← slide left | — |
| 23 | dog-switcher | dashboard | 반려견 선택 | ↓ close sheet | — |
| 24 | dog-switcher | dog-add | "+ 반려견 추가" | → slide right | — |
| 25 | dog-add | dashboard | 저장 완료 | ← pop to root | — |
| 26 | settings | subscription | "구독 관리" 탭 | → slide right | — |
| 27 | settings | dog-profile | "프로필 편집" 탭 | → slide right | — |
| 28 | settings | login | "로그아웃" 확인 | → replace | — |
| 29 | subscription | dashboard | IAP 결제 성공 | ← pop to root | — |
| 30 | ops-today | 개별 기록 모달 | 반려견 행 탭 | ↑ bottom sheet | role(trainer+) |
| 31 | (어디서든) | PRO 업셀 시트 | Skeleton/잠금 영역 탭 | ↑ bottom sheet | — |
| 32 | PRO 업셀 시트 | subscription | "자세히 보기" | → slide right | — |
...
#### 보상형 광고 업셀 퍼널 (Google AdMob Rewarded Ads)
