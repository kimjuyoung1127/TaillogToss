...
Last Updated: 2026-05-15 (KST) — IAP pending-order recovery hardening applied and re-verified on DEV_LOCAL. Legacy non-catalog test SKUs and orders already persisted as terminal failure (`grant_failed/refunded` + `NOT_FOUND/FAILED/REFUNDED`) are now dismissed from SDK pending recovery instead of re-triggering `verify-iap-order` on every startup, and a local suppression marker now prevents repeated cleanup attempts when the Toss SDK keeps returning the same fake pending order. Two cold launches on Sandbox confirmed that the old `verify-iap-order 502` startup noise is gone; the remaining signal is a benign `stale pending order suppressed` log for `sku_106`. DB check confirmed those legacy rows live only under the old mock user `mock_stable_user_001`, not the current active org-owner account. Previous: Training academy 1차 완화 적용: academy 진입 전면광고를 제거하고, 잠긴 유료 커리큘럼도 상세 진입 후 제목 + Day 1 일부 미리보기를 항상 볼 수 있게 유지했으며, academy 하단 PRO 배너를 강한 판매형에서 선택형 안내 톤으로 낮췄다. DEV_LOCAL adb 검증이 실제 사용자 흐름과 일치하도록 training 잠금/미리보기의 `__DEV__` 우회도 제거했다. 이전: 범용 하네스의 planning/evidence/handoff 패턴을 TaillogToss용 최소 agent-layer로 이식했다. `HARNESS-MANIFEST.yaml`, `docs/ref/PROJECT-PLAN.md`, `docs/ref/STACK-PROFILES.md`, `docs/ops/agent-orchestration.md`, `.claude/commands/{intake,impact-map,evidence-review,handoff,profile-recommend}.md`, `scripts/check-{agent-layer,planning,doc-consistency,harness}.sh`를 추가해 세션 분류→영향범위→증거→핸드오프 흐름을 표준화했다. 이전: 자동화 오케스트레이션 정리 완료. 기본 실행은 `morning`, `AI data`, `nightly`, `weekly` 4개 오케스트레이터로 제한하고, 개별 TASK 프롬프트는 하위 작업으로 정렬했다. `taillog-ai-data-orchestrator`는 synthetic gen → Telegram review 순서로 AI-TRAIN-001 루프를 묶는다. `skills-web-enrichment-7day`는 앱 런타임 훈련데이터를 바꿀 수 있어 수동/DRY_RUN 전용으로 내렸고, nightly vision-labeling은 스키마/권한 검증 전까지 기본 실행에서 제외했다. 이전: AI-TRAIN-001 텔레그램 검수 재료 수집 v1 추가. 기존 플라이휠 하드닝(`coaching_synthetic_log.coaching_ids` UUID[] 정렬, 구조화 품질점수, 관리자 review API)에 이어, 자동화가 쓸 수 있는 `GET /api/v1/coaching/admin/training-candidates` 후보 목록과 `candidate-payload` API를 추가했다.
