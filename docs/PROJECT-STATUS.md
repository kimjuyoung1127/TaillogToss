...
Last Updated: 2026-02-28 21:19 (KST)
...
| `verify-iap-order` | v12 | false | 실기기 `POST 200` + `toss_orders` 영속화(2건) 확인 |
...
| IAP-001 | 결제 | In Progress | 구독 화면, useIsPro, verifyAndGrant, Edge v12, iap.test 9케이스, 서버 3시나리오 증적 + DB 영속 확인 | 실기기 결제 UI 3시나리오 증적 정리 |
| MSG-001 | 알림 | In Progress | Edge v9, 쿨다운, noti_history 영속, 우회차단, 테스트 통과 | Sandbox 실발송 |
